...
from models import cnn, resnet, res2net, resnext, sk_resnet, resnest, lstm, dilated_conv, depthwise_conv, shufflenet, vit, dcn, channel_attention, spatial_attention, swin,add,mamba,PHemo
...
#========读取数据=========
    ...
    # X_train.shape 是 (batch, 1, features, 1)
    feature_len = X_train.shape[2]
    # 专为PHemo
    #model = model_dict[args.model](dropout_rate=0.5, units=256, n=4, in_features=feature_len, num_classes=3).to(device)
#=========可改
    model = model_dict[args.model](dropout_rate=0.6, units=256, n=4, in_features=feature_len, num_classes=3).to(device)
